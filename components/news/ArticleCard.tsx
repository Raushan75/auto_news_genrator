...
export function ArticleCard({
  ...
  return (
    <Link
      href={href}
      className="group flex flex-col bg-card border border-border rounded-xl overflow-hidden hover:border-border/60 hover:-translate-y-1 transition-all duration-200 hover:shadow-card-hover hover:no-underline"
    >
      {/* Thumbnail */}
      <div className="relative w-full h-44 bg-muted overflow-hidden shrink-0">
        ...
        {article.isFeatured && !article.isBreaking && (
          <div className="absolute top-3 left-3 bg-primary text-primary-foreground text-[10px] font-bold tracking-wider uppercase px-2 py-1 rounded shadow-lg">
            ★ Featured
          </div>
        )}
      </div>
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          {article.isAiRewritten && (
            <span className="text-[10px] font-semibold bg-blue-500/10 text-blue-400 border border-blue-500/20 rounded-full px-2 py-0.5 flex items-center gap-1 shrink-0">
              <span className="w-1 h-1 rounded-full bg-blue-400 animate-pulse-dot" />
              AI
            </span>
          )}
        </div>
