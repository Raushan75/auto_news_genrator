...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const cfg = getCategoryConfig(category.toUpperCase());
  return {
...
export default async function CategoryPage({ params }: Props) {
  const { category } = await (params as any);
  if (!VALID.includes(category.toLowerCase())) notFound();
  ...
  return (
    <div className="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="mb-8">
        <div
          className={`inline-flex items-center gap-2 text-sm font-bold tracking-widest uppercase mb-3 ${cfg.color}`}
        >
          <span>{cfg.emoji}</span>
          <span>{cfg.label}</span>
        </div>
        ...
        <p className="text-muted-foreground mt-2">
          Latest {cfg.label.toLowerCase()} stories, breaking news and analysis
        </p>
      </div>
      ...
      <AdSlot
        slot={`${category}-top`}
        size="leaderboard"
        className="mb-8 hidden sm:flex mx-auto"
      />
